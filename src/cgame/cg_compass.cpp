...
void __cdecl CG_CompassRegisterDvars()
{
    ...
    minm.value.max = 60.0;
    minm.value.min = 0.0099999998;
    compassRadarUpdateTime = Dvar_RegisterFloat("compassRadarUpdateTime", 4.0, minm, 0x81u, "Time between radar updates");
    minn.value.max = 10.0;
    minn.value.min = 0.0099999998;
    ...
    minbj.value.max = 10.0;
    minbj.value.min = 0.0099999998;
...
double __cdecl CG_FadeCompass(int localClientNum, int displayStartTime, CompassType compassType)
{
    ...
    if (compassType)
        return 1.0;
    v4 = hud_fade_compass->current.value * 1000.0;
    return CG_FadeHudMenu(localClientNum, hud_fade_compass, displayStartTime, (int)(v4 + 9.313225746154785e-10));
}
