                                     ...
                                     S32  discard_all_pixels )
{
  ...
  if ( rad_image->texture_count < rad_image->total_textures )
  {
    ...
    if ( ! SUCCEEDED( rad_image->d3d_textures[ rad_image->texture_count ]->LockRect( 0, &locked_rect, NULL, discard_all_pixels ? D3DLOCK_DISCARD : 0 ) ) )
    {
      goto err;
    }
                                    ...
                                    S32 discard_old_pixels )
{
  ...
  if ( rad_image->texture_count == 0 )
  {
    if ( out_pixel_buffer || out_buffer_pitch )
    {
      ...
      if ( FAILED( rad_image->texture->LockRect( 0, &locked_rect, NULL, discard_old_pixels ? D3DLOCK_DISCARD : 0 ) ) )
      {
        return 0;
      }
                                ...
                                S32  discard_all_pixels )
{
  ...
  if ( rad_image->lin_texture )
  {
    return Lock_RAD_3D_image_lin( rad_image, out_pixel_buffer,
        out_buffer_pitch, out_surface_type, src_x, src_y, src_w, src_h, discard_all_pixels );
  }
  else
  {
